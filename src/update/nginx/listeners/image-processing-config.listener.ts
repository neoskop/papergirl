...
export class ImageProcessingConfigListener extends ConfigListener {
  ...
  protected getConfigLines(meta: Meta): string[] {
    ...
    return [
      `location ~* "^/(?<path>.+)-(?<width>\\d+)x(?<height>\\d+)\\.(?<ext>(jpg|jpeg|png|svg|svgz|gif|webp|avif))$" {
      ...
      `location ~* "^/(?<path>.+)-(?<width>\\d+)w\\.(?<ext>(jpg|jpeg|png|svg|svgz|gif|webp|avif))$" {
      resolver 127.0.0.1:53 ipv6=off;
      set $upstream papergirl-image-proxy:8565;
      ...
      `location ~* "^/(?<path>.+)-(?<height>\\d+)h\\.(?<ext>(jpg|jpeg|png|svg|gif|webp|avif))$" {
    resolver 127.0.0.1:53 ipv6=off;
    set $upstream papergirl-image-proxy:8565;
